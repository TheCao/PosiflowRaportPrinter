...
public:
	Raport(std::string filename ,std::vector<double> *measurements=NULL, std::vector<double> *leakages=NULL, std::vector<std::vector<double>> *histerezis=NULL, unsigned *valveNo=0, std::string directory = "", PDF *p=NULL):
		mFileName(filename), mFileDirectory(directory), mPdf(p), mMeasurements(measurements), mLeakages(leakages), mHisterezis(histerezis), mValveNo(valveNo)
		{
			mPdf = new PDF();
			createRaport();
		};
